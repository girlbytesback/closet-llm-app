...
**`garments/GARMENT_1.jpg` was renamed to `GARMENT_1.jpeg`.** `extract.run` only
...
`garments.json` and `answer_key.py` rekeyed to `.jpeg`, which is what moved the
...
`garments/` and `garments.json`. Regenerated with `closetllm match --out`; rerun
